...
def run_day_analysis(date, intake, profile, agent2_summary, prompt_template, output_dir="output/p01"):
    ...
    response = run_agent3_analysis(prompt)

    print(f"\n🟦 Raw response from LLaMA for {date}:\n{response}...\n")

    output_path = os.path.join(args.folder, f"agent3_{date}.txt")
...
if __name__ == "__main__":
    ...
    intake_path = os.path.join("output/p01", f"{args.date}_intake.json")
    profile_path = os.path.join("agent1", "user_profile.json")
    agent2_path = os.path.join("output/p01", f"agent2_{args.date}.json")
    ...
    run_day_analysis(args.date, intake, profile, agent2_summary, args.prompt_template)
